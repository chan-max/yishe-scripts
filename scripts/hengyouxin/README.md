...
#### 查看上传日志
```bash
npm run hengyouxin:upload-logs
```
...
- 使用时间戳格式确保精确的时间范围
- 自动处理分页，爬取所有符合条件的素材
- **使用单独的日志文件夹**：每个爬取任务使用独立的时间戳命名日志文件
- **日志文件位置**：`yesterday_logs/` 目录下，格式为 `yesterday_YYYY-MM-DDTHH-MM-SS-sssZ.json`
...
#### 4. 日志管理功能
- **主日志文件**：`crawl_log.json` - 用于基本爬取和批量爬取
- **昨天日志文件夹**：`yesterday_logs/` - 专门存储前一天爬取的日志
- **日志查看命令**：`npm run hengyouxin:yesterday-log` - 查看所有昨天爬取的日志文件
- **日志文件信息**：显示创建时间、文件大小、素材数量等详细信息

#### 5. COS 上传和服务器同步功能
- **自动下载**：自动下载素材图片到本地临时文件
- **COS 上传**：自动上传到腾讯云 COS，路径为 `hengyouxin/素材名称.jpg`
- **服务器同步**：自动同步到设计服务器，支持素材管理
- **上传日志**：详细记录 COS 上传和服务器上传的日志
- **失败重试**：上传失败时记录详细错误信息

#### 6. 飞书通知功能
- **成功通知**：爬取完成后发送详细的成功统计信息
- **失败通知**：遇到错误时发送错误原因和处理建议
- **实时反馈**：包含处理数量、成功/失败统计、耗时等信息
- **错误处理**：认证失败、网络错误等都会发送相应通知
...
### 快速部署（推荐）
查看 [QUICK_DEPLOY.md](./QUICK_DEPLOY.md) 了解3步快速部署流程

### 详细部署指南
查看 [DEPLOYMENT_GUIDE.md](./DEPLOYMENT_GUIDE.md) 了解完整的部署和管理流程
...
- **2025-01-31**：新增服务器部署功能
  - 添加一键部署脚本 `deploy.sh`
  - 添加快速部署指南 `QUICK_DEPLOY.md`
  - 添加详细部署指南 `DEPLOYMENT_GUIDE.md`
  - 支持PM2自动安装和配置
  - 支持开机自启动
- **2025-01-31**：新增 COS 上传和飞书通知功能
  - 添加自动下载和上传到腾讯云 COS 功能
  - 添加自动同步到设计服务器功能
  - 添加飞书通知功能，支持成功和失败通知
  - 添加详细的上传日志记录
  - 添加查看上传日志的命令 `npm run hengyouxin:upload-logs`
- **2025-01-31**：新增昨天爬取日志管理功能
  - 为前一天素材爬取创建单独的日志文件夹 `yesterday_logs/`
  - 每个爬取任务使用时间戳命名的独立日志文件
  - 添加查看昨天日志的命令 `npm run hengyouxin:yesterday-log`
  - 显示日志文件的详细信息（创建时间、文件大小、素材数量等）
